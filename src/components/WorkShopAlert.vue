<template>
    <div
        class="invisible fixed right-0 bottom-0 z-50 w-[35%] rounded-md border border-white/25 bg-[rgba(20,21,20,.9)] p-2 shadow-2xl backdrop-blur-sm md:visible lg:w-[30%] lg:p-4 xl:w-[27%]">
        <div>
            <p class="text-center text-xl font-bold uppercase 2xl:text-3xl">
                <span class="text-[#1495FE]">Бесплатный</span> Воркшоп
            </p>
            <ul
                class="mt-2 font-['TT_Octosquares'] text-base lg:text-lg xl:mt-5 [&_span]:font-semibold [&_span]:text-[#1495FE]">
            ...
            <div v-if="!isOpenForm" class="flex justify-center">
                <button
                    ...
                    class="mt-5 rounded bg-[#1495FE] px-4 py-1 text-center text-xl hover:bg-[#0067C3] lg:px-6 xl:py-2">
                    Записаться
                </button>
            </div>
            <form @submit="onSubmitHandler" v-else class="mt-5">
                ...
                <button class="rounded-sm bg-[#38a1f8] px-4 py-1 hover:bg-[#1495FE] lg:px-6 xl:py-2 2xl:text-xl">
                    Записаться
                </button>
            </form>
        </div>
